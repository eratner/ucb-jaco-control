...
TEST(PIDComputedTorqueControl, testPDControl)
{
  ...
  ucb_jaco_control::PIDComputedTorqueController<2> controller({100., 100.},
                                                              {  0.,   0.},
                                                              { 20.,  20.},
                                                              dynamics,
                                                              desired_trajectory);
...
TEST(PIDComputedTorqueControl, testPIDControl)
{
  typedef Eigen::Matrix<double, 4, 1> StateVector;
  typedef Eigen::Matrix<double, 2, 1> ControlVector;
    ...
    [&] (double t, const StateVector& state, const ControlVector& control)
  {
    ...
    // Add a constant disturbance torque.
    Eigen::Matrix<double, 2, 1> dist;
    dist << 1.0, 1.0;

    // Assuming no control input.
    Eigen::Matrix<double, 2, 1> acc = M_inv * (control - dist -
